    ...
    if server {
        ...
        let res = std::panic::catch_unwind(move || {
            ...
            if let Some(w) = &mut game.world {
                ...
                let _player = w.ecs.create_entity()
                    ...
                    .with(GameEntity)
                    .with(PhysicsEntity { on_ground: false, gravity: 0.2, edge_clip_distance: 2.0, collision: true })
                    .with(Persistent)
        ...
        if let Some(w) = &mut game.world {
            ...
            let player = w.ecs.create_entity()
                .with(Player { movement: PlayerMovementMode::Free })
                .with(GameEntity)
                .with(PhysicsEntity { on_ground: false, gravity: 0.5, edge_clip_distance: 2.0, collision: true })
                .with(Persistent)
                .with(Position{ x: 0.0, y: -20.0 })
                .with(Velocity{ x: 0.0, y: 0.0 })
                .with(Hitbox { x1: -6.0, y1: -10.0, x2: 6.0, y2: 10.0 })
