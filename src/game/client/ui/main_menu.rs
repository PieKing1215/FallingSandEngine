...
impl MainMenu {
    fn draw_worlds(tree: &WorldTreeNode<PathBuf, (PathBuf, WorldMeta)>, ui: &imgui::Ui) -> Option<PathBuf> {
        match tree {
            WorldTreeNode::Folder(p, ch) => {
                // TODO: actually implement collapsing
                if !ui.button(&*im_str!("{}", p.file_name().map_or_else(|| "..", |o| o.to_str().unwrap_or("!! NON UTF-8 !!"))), [200.0, 20.0]) {
                    ui.indent();
                    for tr in ch {
            ...
            if ui.button(&*im_str!("{}\n{} - {}", 
                    m.name, p.parent().expect("World file missing parent folder ??").file_name().map_or_else(|| "..", |o| o.to_str().unwrap_or("!! NON UTF-8 !!")), 
                    m.last_played_time.to_string()), [300.0, 40.0]) {
                    return Some(p.clone());
                }
            },
        }
            ...
            match &self.state {
                MainMenuState::Main => {
                    if ui.button(im_str!("Singleplayer"), [100.0, 50.0]) {
                        ...
                        log::debug!("{:?}", worlds);
                        let metas = game::common::world::World::<ClientChunk>::parse_file_tree_metas(worlds).expect("World meta parse failed");
                        log::debug!("{:?}", metas);

                        new_state = Some(MainMenuState::WorldSelect {
                            context: metas,
                        });
                    }
