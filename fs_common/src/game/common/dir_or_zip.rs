...
struct ZipFileGuard<'a> {
    guard: RwLockWriteGuard<'a, ZipArchive<fs::File>>,
    by: ZipBy,
}
...
impl ReadFile for ZipFileGuard<'_> {
    fn file_stem(&mut self) -> Option<String> {
    ...
    fn read_to_string(&mut self) -> Result<String, ReadFileError> {
        ...
        ReadFile::read_to_string(&mut file?)
    }
}

impl ReadFile for ZipFile<'_> {
    fn read(&mut self) -> Result<Vec<u8>, ReadFileError> {
        let mut buf = vec![];
